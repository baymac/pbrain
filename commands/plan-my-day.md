...
**Habit reconcile is delegated.** After the plan is written (and on `update`), the script emits a `HABIT SCAN` block (from the habit module): scan today's vault entries for habit evidence → mark states, and realign today's one-shot reminders to the planned times. Follow it silently. Do not hand-write habit-marking or reminder logic here — `/habits` owns it.
