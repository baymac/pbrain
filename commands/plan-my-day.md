...
**Tasks moved to /plan-my-work.** /plan-my-day lays out LIFE anchors + empty work blocks only; it no longer proposes or assigns tasks and writes no task log. After it confirms the day, nudge: "run /plan-my-work to fill the blocks." If the user runs `plan-my-day task …`, the script emits `PLAN_MY_DAY_TASK_MOVED` — tell them to use `/plan-my-work task add|remove|list` instead (that's where mid-day work edits live now).
...
- Work blocks in "Today at a glance" are GENERIC PLACEHOLDERS ("Block N — focus work"). /plan-my-work fills them with real tasks from Plane and writes the "## Work tracker". /plan-my-day writes no task log.
