---
description: Monthly synthesis — reads all weekly reviews from the current month, 3 reflection questions, writes to life/monthly-tracking/YYYY-MM.md. Drives monthly-goals versioning (commit closing month → mint next month's draft derived from the goals profile, priority only, one goal at a time). Optional goals-profile hygiene pass (move mostly-done to maintenance_mode, drop finished).
---
...
The script emits `MONTHLY_REVIEW_SESSION` with all the month's weekly reviews inlined plus the CORE PROFILES (goals/work/goals-library, diet, fitness, habits — versioned). Follow the INSTRUCTIONS exactly. Key hard rules:
...
- **Monthly goals one by one.** Derive each goal from the goals profile, confirm inclusion + milestone, no batch approvals.
- **Goals-profile hygiene.** Offer the pass once — move mostly-done to maintenance_mode, drop finished. Never force it.
