...
- **PB-85:** the day's `## Today at a glance` (from `/plan-my-day`) and the standalone `## Work tracker` (from `/plan-my-work`) are INDEPENDENT — the day may have either, both, or neither. Close whichever exist (you're handed `glance_present`/`work_tracker_present`), and skip the absent one silently. Fill the `## Work tracker` actuals (Status/Started/Done at/Time taken/% complete) and prefix happened glance blocks with `✓ `. On a **pmw-only** day (no glance) still close the work side fully and summarize from the tracker; on a **pmd-only** day there's simply no ledger to reconcile. Reconcile manual Plane changes (cancelled/done) back into the tracker (Step 4k). No sibling close files.
- Propagate diet + fitness actuals silently from the answers; recompute diet totals.
- **Mark the scored-habit defaults** from the day's data as the backstop — Work the plan, Train, Eat clean, Sleep well, Deep work — then consolidate. Classify the raw inputs by the rules in the `/habits` spec (`commands/habits.md` → "Default scored habits") and pass the per-habit flag; never pick the score yourself (the habit definitions carry no scoring notes — the script computes). Always pass `--note "<basis>"` — a terse one-line account of **how that day's score came about**, so the habit-tracking Note column shows it: e.g. `--note "6 clean / 0 unclean"`, `--note "77m work / 42m social / 34m entertainment"`, `--note "bed 02:35 vs 00:00 · 5.75h"`, `--note "gym skipped — exhausted from football"`. **If the underlying activity didn't happen that day — training skipped, sleep not tracked — mark it `missed`, NOT a scored 0**: a scored 0 means it happened and earned nothing; a miss means it never happened (blank Count). Idempotent if a command already marked one.
