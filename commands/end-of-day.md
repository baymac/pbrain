...
- **Mark the scored-habit defaults** from the day's data as the backstop — Work the plan, Train, Eat clean, Sleep well, Deep work — then consolidate. These carry no notes: classify the raw inputs by the rules in the `/habits` spec (`commands/habits.md` → "Default scored habits") and pass the per-habit flag; never pick the score yourself. Idempotent if a command already marked one.
