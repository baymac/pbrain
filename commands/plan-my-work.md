...
- `PLAN_MY_WORK_SESSION` — the main flow, with a big data block (plans profile working_style + bed, this week's project goals with `allocation_percent`/`plane_project`, the project registry, a `progress_json` for this week's goal projects, this week's `## Work tracker` rows for carry-forward, and — when `plan_exists: yes` — today's plan with its empty work blocks). Follow the numbered INSTRUCTIONS: **(1)** preflight/standalone, **(2)** run `/project-manager review` and walk thin issues, **(3)** a progress report (per-goal alloc% · Plane pct · tasks done · flags; **% of week's importance met = Σ(alloc%·pct)/Σ(alloc%)**), **(4)** pick projects → renormalize allocation across the blocks (use the `alloc` helper — deterministic, leftover to highest priority), **(5)** pull ready tasks and pack biggest-rock-first (prefer one project per block; honor `last_block_end`), **(6)** write `## Work tracker` + relabel the glance work-block rows, **(7)** confirm tightly. **Standalone** (`plan_exists: no`): don't build a whole-day timeline — ask focus hours, compute blocks now → bed with the `blocks` helper, and write a minimal file (short work-only glance + the tracker).
