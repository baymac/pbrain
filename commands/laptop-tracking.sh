...
try:
    ...
    rows = con.execute(
        "SELECT started_at, ended_at, app_bundle_id, app_name, raw_host, attribution "
        "FROM tracker_segments WHERE occurred_on=? ORDER BY started_at", (day,)
    ).fetchall()
    con.close()
...
for s, e, bid, name, host, attr in rows:
    ...
    # attribution is only meaningful for browser rows (skip plain apps).
    if (attr or "") != "not_browser":
        attr_secs[attr or "ok"] = attr_secs.get(attr or "ok", 0) + dur
    if dur > 0:
        segs.append((s, e))

if segs:
    first = min(s for s, e in segs)
...
if not segs:
    ...
    # Attribution rollup — only surfaced if SOME browser time wasn't a clean
    # domain capture, so unattributable time is explained, never silent (T2).
    noisy = {k: v for k, v in attr_secs.items() if k != "ok"}
    if noisy:
        L.append("")
        L.append("## Browser attribution")
        L.append("")
        btot = sum(attr_secs.values())
        labels = {"ok": "domain captured", "tcc_denied": "permission not granted",
                  "timeout": "browser timed out", "non_web": "no web page (blank/file/settings)"}
        for k, v in sorted(attr_secs.items(), key=lambda x: -x[1]):
            p = (100 * v // btot) if btot else 0
            L.append("- %s: %s (%d%%)" % (labels.get(k, k), hm(v), p))

L.append("")
L.append("_Generated by /laptop-tracking from tracker.db — local data, domain-level only._")
content = "\n".join(L) + "\n"
