...
try:
    ...
    rows = con.execute(
        "SELECT started_at, ended_at, app_bundle_id, app_name, raw_host, %s, attribution "
...
L.append("")
L.append("_Generated by /laptop-tracking from tracker.db — local data (host + path, query strings dropped); never synced._")
content = "\n".join(L) + "\n"
