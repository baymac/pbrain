...
**Default scored habits.** As soon as the profile they key off is committed, five scored habits seed themselves — each a 0.0–1.0 unit score you never pick by hand (every scored habit shares this scale; only the model differs — the model only classifies your raw inputs, the lib computes the number):
