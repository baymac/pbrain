...
`add --name "X" --direction at_least|at_most --schedule daily|weekdays|interval|monthly [schedule args] [--unit "L"] [--measure-target N] [--target N] [--components "A; B=2; C"] [--priority …] [--notes "…"]` — schedule args by kind: `weekdays` → `--days mon,wed,fri` or `--times-per-week N [--start-day mon]`; `interval` → `--every-days N [--start-date YYYY-MM-DD]`; `monthly` → `--days-of-month 1,16` or `--times-per-month N [--start-dom D]`. `--components` attaches a checklist scoring spec (weight after `=`, default 1; defaults `--measure-target` to 100 so the score persists). (Legacy `--type daily|weekly|monthly [--target N]` still works — it maps to a schedule.)
...
**Default scored habits.** As soon as the profile they key off is committed, five scored habits seed themselves — each a 0–100 score you never pick by hand (the model only classifies your raw inputs; the lib computes the number):
...
- **Deep work** (`focus_ratio`, target 75 — seeded once **laptop tracking is set up** *and* a **plans profile** exists): scores how focused your scheduled work blocks actually were — not whether the tasks got done (that's *Work the plan*), but whether the time went to work vs. distraction. At `/end-of-day` it maps the day's laptop activity onto the plan's work-block windows and scores `100 · work / (work + distraction)` of *active* minutes; AFK time is reported but never penalized. Which domains/apps count as work vs. social vs. entertainment comes from a reusable [category map](laptop-tracking.md) that grows as you confirm new sites. Scheduled on your plan's workdays (weekdays minus rest days), so weekends aren't counted as missed — `mark --name "Deep work" --focus '{"work":130,"social":35,"entertainment":15,"neutral":10}'`.
