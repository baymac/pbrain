...
- **Eat clean** (`slip_ladder`, seeded once a **diet profile** exists): score steps down a ladder by the number of slips — `mark --name "Eat clean" --good <clean meals> --bad <unclean meals>` (slips = unclean meals plus any shortfall under the clean-meal target of 3; ladder `[1.0, 0.6, 0.3, 0]`). The diet journal's extraction block classifies the meals; the profile rule owns the formula.
