...
Your preferences live in the vault under `.pbrain/` (migration 0001 copied any older `~/.config/pbrain/` files across), so they sync to every device with the rest of your vault. Quality fixes — bugs and improvements that help *everyone* — don't belong in your personal vault; the pass offers to file those as GitHub issues against `baymac/pbrain` instead.
...
| **Quality fix** | A bug or improvement that helps *everyone* | A GitHub issue on `baymac/pbrain` (one-line fallback note in `$VAULT_DIR/.pbrain/<command>/quality-log.md`) | n/a (filed upstream) |
...
| `PBRAIN_PREFS_DIR` | Preferences ROOT (`_global/prefs.md` + per-command `<cmd>/prefs.md`; also holds the quality-log fallback) | `$VAULT_DIR/.pbrain` |
