...
5. **Focus hours → block layout** — "how many focused hours from now?" The planner computes work blocks around your **life anchors** (calendar events, the fitness session, meal slots, the walk/wind-down/bed, habit reminder times) and lays them as **fixed `session_length_min` units** with **generic placeholders** ("Block N — focus work"): it fits as many full blocks as the day holds and flexes the *count*, never squeezing a block below its set length to plug a gap — a gap too small for a full block + break is left as rest, a block is trimmed only where it meets a hard anchor, and only the final wind-down block may run short. A scheduled fitness session is laid as **one combined block** (commute-in + session + commute-out) plus a separate post-activity settle, sized from the fitness journal's actual duration. Breaks sit at the band median by default. **Work comes first** — every usable gap is filled with focus blocks from the earliest slot (including a pre-anchor morning gap, e.g. before a 14:30 gym), so the first block lands as early as the anchors allow and a long *invented* idle run right after gym+lunch never happens. **No invented stacked rest** (never add a new rest row next to another — they merge and the freed slot becomes work), but a **profile-defined fixed rest** — the post-activity settle and the **post-lunch nap** — is always kept at its set length (protected like a meal). The **Snack is treated as a break**, not a counted meal: one flex rest row placed ≈1–2 blocks after lunch and well before dinner, not pinned to the profile's Snack clock time. On a **late wake (after 11:00)** the morning compresses to a 1-hour run before the first block — 30 min routine (halved) + 30 min breakfast (kept). The table is **gap-free and overlap-free** — each row's end equals the next row's start. Anchors are life structure only — work is never an anchor; it fills the gaps the anchors leave.
