...
How `scripts/gbrain-sync-wrapper.sh` works, why it's shaped this way, and when to consider switching gbrain from PGLite to Postgres / Supabase.

---

## ⚠️ Current state: DEGRADED MODE (launchd disabled)

As of 2026-05-24, the scheduled launchd sync is **unloaded**. `gbrain sync` (and the `gbrain call sync_brain` proxy when no serve is running) hangs indefinitely after a `89 → 92` schema migration. See `docs/gbrain-bug-report.md` for the full repro to send upstream.
...
`scripts/gbrain-sync-wrapper.sh` is invoked by the launchd job `~/Library/LaunchAgents/com.pbrain.sync.plist` every 30 minutes (`StartInterval=1800`).
...
- `scripts/gbrain-sync-wrapper.sh` — the wrapper
