...
- **`/pbrain-codex-install` installer hardening:** the `AGENTS.md` update step now skips the write (rather than overwriting with empty content) if the file exists but cannot be read due to permissions; `os.makedirs` is now guarded against an empty dirname when `CODEX_HOME` resolves to a bare filename.
