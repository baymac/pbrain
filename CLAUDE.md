...
| `/plan-my-work` | the WORK layer, run after `/plan-my-day`; pulls ready Plane tasks into the empty blocks → `## Work tracker`; `task` subcommand. The auto-pull session **requires Plane** (emits `PLAN_MY_WORK_NO_PLANE` when unconfigured); `task add\|remove\|list` edits work without it | `life/daily-planning/<date>.md` |
...
| `/project-manager` | the Plane commander and **sole** project backend (absorbs `/init-plane` setup + Plane ops: projects/ready/progress/review/enrich/move/…); needs a vault. Task planning + project progress require Plane; ops emit `PM_NOT_CONFIGURED` when it isn't set up | Plane (no vault write) |
