...
| `agent-work/daily-grooming/` | `/project-manager groom` outputs (PB-94) — per-day grooming data: **todo-only** triage (backlog is the user's staging area, never touched), the **ordered run queue** (blockers ahead of blocked), and `/plan-my-work`'s per-id auto-work outcomes. iCloud-synced for morning review. One `<date>.md` per day. |
