...
├── lib/plane.py                 ← pbrain ↔ Plane backend (multi-project, stdlib urllib); the sole project source; seams + config in its header
...
├── lib/migrations/              ← the ordered migrations 0001–0007 (each script self-documents); see "Versioned profiles + migrations" below
...
| `/fitness-journal` | flexible daily fitness **logger** — describe your session in plain words, it derives the activity's per-activity KPIs (gym→sets, swim→distance, dance→min), tolerates partial logs, then OFFERS to plan (never prescribes). KPIs live in the fitness library + are user-extensible; an activity missing `kpis` derives archetype defaults on the fly (no migration — graceful fallback + lazy save). fitness profile + activity library (fixed weekly days, per-activity KPIs) + per-activity profiles (gym Block/Day kept as OPTIONAL planning reference); `activity:`/`focus:`/`sleep_*` frontmatter + Train habit + `fitness-reconcile` preserved; training-gap band now a planning hint only | `fitness/daily-tracking/` |
| `/finance` | personal financial tracker (analytics, not a spend limiter — no per-category caps); versioned finance profile (currency/categories/accounts/investments/recurring/income/savings+contingency targets) + living merchant→category library; live monthly report `YYYY-MM.md` updated by **LLM-assisted ingestion** (paste statement/CSV/list → parse → dedupe on date+amount+normalized-desc → categorize exactly-one with `other` fallback → recompute). `profile`/`ingest` subcommands; diet/fitness categories label-link to the journals (v1) | `life/finance-tracking/<YYYY-MM>.md` |
...
| `/plan-my-work` | the WORK layer, run after `/plan-my-day`; owns goals → allocation → blocks → `## Work tracker`. Grooming/triage is **delegated to `/project-manager`** (executor mode), not inline; weekly-goal-driven progress (+ monthly read). Thin dispatcher → `templates/plan-my-work/*.txt`; `task add\|remove\|list` subcommand. Auto-pull **requires Plane** (`PLAN_MY_WORK_NO_PLANE` when unconfigured); `task …` edits work without it | `life/daily-planning/<date>.md` |
...
- `pbrain_emit_prefs <cmd>` — near the top of output. Injects `$VAULT_DIR/.pbrain/_global/prefs.md` (cross-command standing prefs) then `.pbrain/<cmd>/prefs.md` (per-command). Apply both for the session; they override defaults where they conflict — including built-in suggestions/nudges. Prefs live IN THE VAULT so they sync across devices (`PBRAIN_PREFS_DIR` overrides the root).
- `pbrain_emit_self_improve <cmd> [profile-file] [profile-label]` — at the end of output. Emits a `SELF-IMPROVE CHECK` block. **Fire only on explicit feedback this session — stay silent on neutral sessions.** Preferences consolidate into `.pbrain/<cmd>/prefs.md` (classified GLOBAL → `_global/prefs.md` vs COMMAND by whether they span commands); quality fixes append to `.pbrain/<cmd>/feedback.md` with an optional `gh issue` offer. With a `profile-file`+`profile-label` (the profile-owning commands), it also gains a PLAN UPDATE route (proposed per-change, written only on an explicit yes; committed versions are immutable → structural changes go through `profile new`/`commit`). `/weekly-review` does profile improvements via its richer Step 4 instead (no profile args).
