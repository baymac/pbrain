...
├── lib/reminders.sh                    ← two reminder backends. /remind: Apple Calendar layer (pbrain_calendar_add/list/delete/today/rrule/access + pbrain_calendar_app_build). /remind-blocking: cron (pbrain_cron_next) + blocking-overlay build/show (pbrain_overlay_build, pbrain_overlay_show) + the poller tick (pbrain_reminders_tick — fires/defers/misses due occurrences over the reminder_schedules+reminders model, advancing the series on fire-or-miss). pbrain_notify/pbrain_notify_build remain only as the overlay's no-swiftc notification fallback (now a plain notifier — the old Snooze/Cancel+SQLite write-back was dropped with /remind's DB queue)
