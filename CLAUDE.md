...
| `/plan-my-work` | the EXECUTION layer (PB-94) — takes **ONE Plane issue id** and drives it through a fixed **5-stage pipeline** (`auto:plan`→`auto:implement`→`auto:test`→`auto:ship`→`auto:land`), auto-advancing each stage whose label is present and **parking** (WIP-commit + push + a `pbrain park:` issue comment) at the first stage without it; **stop-at-first-gap**, later labels don't imply earlier. **State is Plane-only** — no `## Work tracker`, no daily planning, no `task add/remove/list`; reads via `/project-manager spec <id> --read`, writes status/log via `move`/`comment`, and **writes nothing to the vault**. No id → `PLAN_MY_WORK_NEEDS_ID` (it asks; never selects tasks — that's groom's job). Dependency-aware/**multi-loop**: a `blocked_by` issue runs its blocker first; a parent with open sub-issues drives each child (one branch/PR/merge each, parent closed last, PB-81). The only command that touches code/branches/PRs/merges (isolated worktree off fresh `origin/<base>`); land is gated on **CI-green** (red hard-stops regardless of `auto:land`) + `auto:land`-or-typed-confirm, **no release**. NL form (PB-96): `fix X`/`work on pb96` normalize to the same path; `task execute <id>` is a back-compat alias. Requires Plane (`PLAN_MY_WORK_NO_PLANE`) | Plane only (no vault write) |
