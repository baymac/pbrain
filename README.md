...
| `/finance` | Personal financial tracker (analytics, not a spend limiter) — live monthly report (transactions, income, recurring payments, spend by category, balances & net worth, savings rate, contingency runway). First run builds a versioned finance profile (currency, categories, accounts, investments, recurring bills, income, savings + contingency targets) via interview. Paste a statement / CSV / list and it parses → **dedupes** → categorizes (exactly one category each, `other` fallback) → folds it into the month report. Merchant→category memory in a living **category library** | `$VAULT/life/finance-tracking/YYYY-MM.md` |
