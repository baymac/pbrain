...
' "$cmd" 2>/dev/null || true)"
  ...
  printf '%s\n' "NO FUZZY / SHARED-KEYWORD MATCHING (PB-116). The evidence must genuinely BE"
  printf '%s\n' "that habit's activity, not merely share a word with its name. A different"
  printf '%s\n' "chore/surface/target that overlaps on a keyword does NOT count — e.g."
  printf '%s\n' "\"washroom cleaning\" / \"did the dishes\" do NOT satisfy a \"Clean room\""
  printf '%s\n' "habit (shared word \"clean\"/\"room\" is not a match); a 5-min walk to the"
  printf '%s\n' "shop does NOT satisfy a \"Walk 30 min\" habit. If the user did a related"
  printf '%s\n' "but DISTINCT activity and there is no habit that genuinely fits it, mark"
  printf '%s\n' "NOTHING — never force-fit it onto the nearest-sounding habit."
  printf '%s\n' ""
